...
/**
 * Class that attempts to connect to Default DB until connection is successful. Task
 * manager is started and sends tasks to HEK puller and image puller and creates
 * a JobRecord object in Monitoring DB every 30 seconds. Monitoring App is
 * started, which creates an HTTP server on port 1278. The app provides a list
 * of JobRecords created in the DB.
 * @author - ahmetkucuk
 * @author - kqian5
 */
public class TaskSchedulerRunner {
	...
	/**
	 * Main method that attempts to connect to Default DB until connection is successful.
