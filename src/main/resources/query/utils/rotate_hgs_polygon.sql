...
BEGIN
    ...
    RETURN st_makepolygon(ST_MakeLine(array_agg(rot_points.rotated))) from (select rotate_hgs_point((ST_DumpPoints(input_polygon)).geom, hours) as rotated) as rot_points;
END;
$BODY$
LANGUAGE plpgsql;